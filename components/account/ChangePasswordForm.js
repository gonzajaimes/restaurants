...
import { reAuthenticate, updateThePassword } from '../../utils/actions'
...
export default function ChangePasswordForm({ setShowModal, toastRef }) {
    ...
    const onSubmit = async() => {
           if(!validateForm()){
            ...
            const resultUpdatePassword = await updateThePassword(newPassword)
            setLoading(false)
    ...
    const validateForm = () => {
        ...
        if (size(confirmPassword) < 6){
            setErrorConfirmPassword( "Debes ingresar una confirmación de contraseña de al menos 6 caracteres.")
            isValid = false
        }
